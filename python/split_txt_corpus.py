...
if arg.balance == 'balanced':
    path = os.path.join(path, arg.balance)
    name = f'{arg.balance}_{arg.lang}_{arg.domain}'
else:
    name = f'{arg.lang}_{arg.domain}'
...
with (
    open(os.path.join(arg.output, f'{name}_train.json'), 'w', encoding='utf-8') as json_train,
    open(os.path.join(arg.output, f'{name}_test.json'), 'w', encoding='utf-8') as json_test,
    open(os.path.join(arg.output, f'{name}_dev.json'), 'w', encoding='utf-8') as json_dev
    ):
    entries = [{'text': line} for line in corpus]
    df = pd.DataFrame(entries).sample(frac=1, random_state=21)
